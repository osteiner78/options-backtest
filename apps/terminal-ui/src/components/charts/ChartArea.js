...
const legends = {
  equity:    `<div class="leg-item"><div class="leg-line" style="background:var(--pos)"></div>Portfolio</div><div class="leg-item"><div class="leg-line" style="background:var(--a2);opacity:0.8"></div>SPY B&H</div><div class="leg-item"><div class="leg-line" style="background:var(--neg);opacity:0.7"></div>Drawdown % <span style="color:var(--dim);font-size:8.5px">(below)</span></div>`,
...
export function renderChartArea() {
  ...
  // Equity tab gets two stacked panes
...
export function drawChart() {
  ...
  if (activeTab === 'equity') {
    ...
    chart = new uPlot(
      {
        ...vixOpts(w, h, params.vix_entry_max),
        hooks: {
          setCursor: [(u) => {
...
function bprOpts(w, h, capVal) {
  ...
  return {
    ...
    series: [{ label: 'Date' }, { label: 'BPR', stroke: a2, width: 2, fill: a2 + '33' }],
    hooks: { draw: [u => drawHLine(u, capVal, neg, `CAP $${Math.round(capVal / 1000)}k`)] },
  };
}
...
function vixOpts(w, h, vixMax) {
  ...
  return {
    ...
    hooks: { draw: [u => drawHLine(u, vixMax, neg, `VIX MAX ${vixMax}`)] },
  };
}
