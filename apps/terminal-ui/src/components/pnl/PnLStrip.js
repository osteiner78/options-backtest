...
// Trade rects for hit-testing on hover: [{x1, x2, trade}]
let _rects = [];
...
export function initPnLStrip() {
  ...
  // Tooltip on hover — listen on the document, filter to svg-pnl
  document.addEventListener('mousemove', (e) => {
    const svg = document.getElementById('svg-pnl');
    const tip = document.getElementById('pnl-tip');
    if (!svg || !tip) return;

    // Check if mouse is over the SVG
    const rect = svg.getBoundingClientRect();
    const inSvg = e.clientX >= rect.left && e.clientX <= rect.right &&
    ...
    tip.innerHTML = `
      ...
      <div class="ctt-row"><span style="color:var(--dim)">P&L</span><span style="color:${pnlColor};font-weight:600">${sign}$${Math.abs(Math.round(pnl)).toLocaleString()}</span></div>
    ...
    // Position tooltip to the opposite side from the bar
    const wrapRect = document.getElementById('pnl-chart-wrap').getBoundingClientRect();
...
export function drawPnL() {
  ...
  if (!results) {
    svg.innerHTML = `<line x1="0" y1="${h * 0.5}" x2="${w}" y2="${h * 0.5}" stroke="${bg3}" stroke-width="0.5"/>`;
    return;
  }
  ...
  // Date range from equity_curve (matches equity chart x-axis)
  const eqDates = Object.keys(results.equity_curve);
  const minTs = dateMs(eqDates[0]);
  const maxTs = dateMs(eqDates[eqDates.length - 1]);
  const span  = maxTs - minTs || 1;

  const trades = results.trades.filter(t => t.exit_date && t.pnl != null);
  const maxAbs = Math.max(...trades.map(t => Math.abs(t.pnl)), 1);

  const PAD = 4;          // px above/below zero line
  const mid = h / 2;
  const halfH = mid - PAD;

  let bars = `<line x1="0" y1="${mid}" x2="${w}" y2="${mid}" stroke="${bg3}" stroke-width="0.5"/>`;

  // Year separator lines (same appearance as main charts)
  const minYear = new Date(minTs).getUTCFullYear();
  const maxYear = new Date(maxTs).getUTCFullYear();
  for (let y = minYear + 1; y <= maxYear; y++) {
    const x = (Date.UTC(y, 0, 1) - minTs) / span * w;
    if (x > 0 && x < w) {
      bars += `<line x1="${x}" y1="0" x2="${x}" y2="${h}" stroke="${dim}" stroke-width="1" opacity="0.5"/>`;
    }
  }

  trades.forEach(t => {
    const entryMs = dateMs(t.entry_date);
    const exitMs  = dateMs(t.exit_date);
    const x1 = Math.max(0, (entryMs - minTs) / span * w);
    const x2 = Math.min(w, (exitMs  - minTs) / span * w);
    ...
    bars += `<rect x="${x1.toFixed(1)}" y="${y.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" fill="${color}" opacity="0.85"/>`;
