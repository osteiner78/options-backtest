...
class SyntheticEngine:
    ...
    def get_gap_open_mark(
        ...
        e.g. 2% overnight gap -> +6% VIX (multiplier = 3).
        """
        vix_open = vix_prev_raw * (1.0 + self.gap_mult * gap_pct)
        sigma_put = apply_skew(
            vix_open, self.delta, "put", self.put_slope, self.call_slope
        )
        sigma_call = apply_skew(
            vix_open, self.delta, "call", self.put_slope, self.call_slope
        )
